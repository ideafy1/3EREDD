...
try:
    app = Client(
        ...
        workers=20  # Increase number of workers
    )
    db = Database()
    ...
    try:
        ...
        if len(args) > 1:
            ...
            if parameter.startswith("file_"):
                file_id = int(parameter.split("_")[1])
                try:
                    file_message = await client.get_messages(Config.DATABASE_CHANNEL, file_id)
                    if file_message:
                    ...
                    return
            elif parameter.startswith("batch_"):
                # Handle batch file retrieval
                try:
                    _, start_id, end_id = parameter.split("_")
                    start_id, end_id = int(start_id), int(end_id)
                    await message.reply_text("Preparing your batch files...")
                    for msg_id in range(start_id, end_id + 1):
                        try:
                            msg = await client.get_messages(Config.DATABASE_CHANNEL, msg_id)
                            if msg:
...
async def main():
    try:
        ...
        print("Bot is running...")  # Add this line for visual confirmation
        await idle()
    except Exception as e:
